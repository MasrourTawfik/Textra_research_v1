Welcome to Textra-AI-Research's documentation!
====================================

.. figure:: docs/Images/LOGO.png
   :width: 60%
   :align: center
   :alt: Textra-AI-Research's logo
   :name: logo

   ..

.. toctree::
   :maxdepth: 2
   :caption: Table of contents:

   docs/Scripts/introduction.rst
   docs/Scripts/pipeline&architecture.rst
   docs/Scripts/arxiv.rst
   docs/Scripts/pdf to images conversion.rst
   docs/Scripts/tables and figures detection.rst
   docs/Scripts/text_extraction.rst
   docs/Scripts/Building the Base Knowledge Graph.rst
   docs/Scripts/kg_base.rst
   docs/Scripts/kg_enhancement.rst
   docs/Scripts/navigating the graph.rst


   
   
   
   
   
         
